...
async fn domain_redirect(
    ...
    // Check if request hostname matches liminal.journey, otherwise redirect to it.
    if req.connection_info().host() != "liminal.journey" && !req.connection_info().host().starts_with("127.0.0.1") {
        return Ok(ServiceResponse::new(
            req.request().to_owned(),
            HttpResponse::TemporaryRedirect()
                .append_header(("Location", "https://liminal.journey"))
                .finish(),
        )
        .map_into_boxed_body());
    }
